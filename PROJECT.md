...
| 2026-08-21 | Fase 3 | **Rediseño integral del Inicio y Lobby (Estilo MTG Arena / Master Duel)**: (1) **LoginScreen modernizado**: Fondo místico con orbes de maná animados, presets rápidos (`Localhost`, `Oficial Beta beta.xmage.today:17171`, `Personalizado`), preview de avatar de usuario con inicial, caja de red compacta y feedback de conexión suave. (2) **LobbyScreen tipo centro de mando**: Barra de navegación superior con estado de red en vivo, pestañas *[ ⚔️ Mesas ]*, *[ 🃏 Mis Mazos ]*, *[ 👥 Comunidad & Chat ]*, perfil de usuario y desconexión rápida. (3) **Tarjetas de mesas de videojuego**: Badges luminosos de estado (*Listo para empezar*, *En partida*, *Esperando*), plazas visuales de jugadores (`👤 Humano`, `🤖 IA`, `⭕ Vacío`), filtros por formato y hero buttons (*+ Nueva mesa*, *▶ Demo IA*). (4) **Gestor de Mazos (`DeckManager.tsx`)**: Visor de cartas con curva y desgloses, selector de mazo activo e **importador de listas en formato MTG Arena / MTGO en texto plano** con persistencia en `localStorage`. (5) **Cajón colapsable de eventos de red**: Registro de depuración minimizado en la esquina inferior. | vitest **194/194** ✅; typecheck ✅; build ✅; Playwright e2e ✅ |
| 2026-08-21 | UI/Fix | **Grid de mesas adaptativo de 3 columnas & Tarjetas enriquecidas (`LobbyScreen.tsx`, `LobbyScreen.css`)**: (1) Badges de metadatos completos (`🏆 Torneo`/`⚔️ Duelo`, `🔒 Privada`, `⭐ Novato`/`Casual`/`Competitivo`, `🏅 Rated`, `👁️ Espectadores` y tiempo transcurrido `⏱️ hace 2m`). (2) Grid de 3 columnas responsivo con altura automática y botonera fija al pie (`flex-wrap: wrap; flex-shrink: 0`), eliminando por completo cualquier desbordamiento o solapamiento vertical al sentarse jugadores o crearse múltiples acciones. (3) Badges de asientos compactos con elipsis en nombres largos. | vitest **194/194** ✅; typecheck ✅; build ✅ |
| 2026-08-21 | Arq/Fix | **Acción centralizada `returnToLobby()` y eliminación de redundancias de salida**: (1) Implementada `returnToLobby()` en `actions.ts` / `store.ts` que notifica al servidor (`quitMatch` o `stopWatching`), limpia la persistencia activa (`clearActiveGame`) y restablece el estado de partida transicionando limpiamente a `phase: 'lobby'`. (2) `GameEndDialog.tsx` ahora invoca `returnToLobby()` para volver al lobby de forma instantánea al concluir el match. (3) Reemplazado el botón roto `Quit` de `GameScreen.tsx` y la lista de 13 botones duplicados de `Sidebar.tsx` por un menú limpio y accesible con confirmación nativa. (4) Eliminados archivos duplicados huérfanos `Sidebar copy.tsx` y `Sidebar copy.css`. (5) Adaptados los selectores de los tests E2E (`start-game.ts`, `best-of-3`, `best-of-5`, `best-of-n`, `chat`, `defeat`, `full-flow`) para la nueva nomenclatura simplificada. | vitest **194/194** ✅; typecheck ✅; build ✅ |
| 2026-08-21 | UI/Fix | **Unificación de la Pila (Stack) a vista homogénea, panel lateral compacto (250px) y corrección de flechas de targeting en la Pila**: (1) **Plantilla unificada en la Pila (`StackZone.tsx`, `StackZone.css`)**: Todos los elementos del stack (hechizos y habilidades, `#1` a `#N`) comparten el mismo layout en cascada vertical: badge numérico/tipo, carta grande (226px) y caja de texto de reglas/efectos formateada con símbolos de maná. El botón `Resolver` queda anclado al pie del elemento activo `#1`. (2) **Panel derecho compacto (250px en `GameScreen.css`)**: Reduce el ancho lateral de 330px a 250px, otorgando 80px extra de espacio horizontal al campo de batalla. (3) **Overlay global de flechas de targeting (`CombatArrowsOverlay.tsx`, `GameScreen.tsx`)**: Reubicado al contenedor macro `.game-body` (`position: relative`), permitiendo que el canvas SVG dibuje flechas doradas luminosas continuas desde las cartas en la pila hacia criaturas, jugadores u otros permanentes en el campo de batalla. (4) **Mejora en `resolveTargetSourceId.ts`**: Resuelve IDs de objetivos desde la pila, mano y permanentes considerando nombres de origen y habilidades. | vitest **195/195** ✅; typecheck ✅; build ✅ |
| 2026-08-21 | Fix | **Resolución de ilustración de reverso (segunda cara / Transform / MDFCs) en `FloatingCardPreview` y `CardPreview`**: (1) `isCardBackFace` y `cardKey` en `cardImages.ts` ahora asignan sufijo único `#back` (`ISD/51#back`, `named:...#back`) al voltear o transformar una carta, evitando colisiones de caché con el anverso. (2) `tryFetch` extrae `card_faces[1].image_uris` de Scryfall y precarga en memoria ambas caras en una sola petición. (3) `FloatingCardPreview.tsx` y `CardPreview.tsx` propagan `isSecondCardFace: true` en `activeCard`, actualizando la ilustración, nombre y reglas en tiempo real al presionar `Shift` o `F`. (4) `CardSlot.tsx` detecta permanentes transformados (`perm.transformed`) en el campo de batalla y refresca su arte. | vitest **197/197** ✅; typecheck ✅; build ✅ |
| 2026-08-21 | Feature/Fix | **Motor universal de renderizado de texto/HTML XMage & Preview interactivo al pasar el cursor (Hover) sobre cartas y jugadores**: (1) **Motor de saneamiento en `FormattedText.tsx`**: Parser universal que procesa etiquetas `<font\b[^>]*>` con cualquier orden de atributos (`color`, `object_id`), elimina de raíz hashes técnicos de 2 a 8 caracteres (`[dcf]`, `[9b4]`, `[373]`) y genera fragmentos estilizados con símbolos gráficos de maná `{R}`, `{U}`, etc. (2) **Hover interactivo sobre cartas (`FloatingCardPreview.tsx`)**: Al pasar el cursor sobre cualquier nombre de carta en el Chat de Partida (`GameChat.tsx`), Chat del Lobby (`ChatBox.tsx`) y Registro/Feed (`ActionFeed.tsx`), se abre instantáneamente la previsualización flotante HD con su arte y texto de reglas. (3) **Hover estilizado para jugadores**: Badges y nombres de jugadores enriquecidos con tooltip nativo `👤 Jugador: [Nombre]` y relieve luminoso. (4) **Integración en Lobby y Feed**: `ChatBox.tsx` y `ActionFeed.tsx` (en modo visual y modo texto crudo) migrados a `FormattedText` con hover activo. | vitest **199/199** ✅; typecheck ✅; build ✅ |
| 2026-08-21 | Fix | **Parser de eventos de creación de fichas (`creates a ... token`) & saneamiento de banners del log (`ActionFeedCard.tsx`, `gameEventParser.ts`)**: (1) Añadido reconocedor estructurado en `gameEventParser.ts` para eventos de creación de tokens (`[Player] creates a [TokenName] token` / `creates N [TokenName] tokens`), asignándole tarjeta visual de habilidad, autor, nombre de ficha y arte en miniatura. (2) `ActionFeedCard.tsx` ahora renderiza descripciones de sistema y de tarjetas visuales a través de `<FormattedText onHover={onHover} />`, eliminando de raíz cualquier tag `<font...>` o hash remanente en las pestañas de Log de la partida. | vitest **200/200** ✅; typecheck ✅; build ✅ |
| 2026-08-21 | Feature | **Biblioteca / Mazo Clickeable & Visor de Cartas Reveladas y Dorso Oficial de Magic (`ResourceBar.tsx`, `PileOverlay.tsx`)**: (1) **Mazo interactivo en tablero**: La pila de la biblioteca (`.library-stack`) se convierte en botón clickeable con efecto hover, indicador numérico y borde dorado luminoso cuando la carta superior está revelada (`player.topCard`). (2) **Visor Modal de Biblioteca (`PileOverlay.tsx`)**: Al hacer clic, abre un visor completo con la secuencia ordenada de cartas de la biblioteca: la carta `#1 (Top)` se muestra boca arriba con su arte HD, costes y badge `★ #1 TOP 👁️` si está revelada (ej. por *Courser of Kruphix*, *Future Sight*, etc.), y el resto de cartas se muestran con el dorso oficial de Magic numeradas secuencialmente (`#2`, `#3`... `#N`). (3) **Hover y Jugabilidad**: Soporte para `FloatingCardPreview` al pasar el cursor sobre cartas conocidas y compatibilidad para lanzar cartas superiores jugables directamente desde la biblioteca. | vitest **203/203** ✅; typecheck ✅; build ✅ |
| 2026-08-21 | UI/UX | **Ampliación HD de cartas y modales para legibilidad directa sin zoom (`PileOverlay.css`, `CardGrid.css`, `LibraryOrderDialog.css`)**: (1) **Visores de Pilas y Mazo (`PileOverlay.css`)**: Ancho de carta incrementado de 140px a **210px** (proporción 210x294px) y ancho de modal a 1200px con scroll horizontal suave, permitiendo leer reglas, costes y texto completo directamente sin necesidad de hover. (2) **Selector de Scry / Surveil / Biblioteca (`LibraryOrderDialog.css`)**: Cartas ampliadas de 110px a **175px** y modal a 1150px. (3) **Cuadrícula de búsqueda y selección (`CardGrid.css`)**: Cuadrícula adaptativa ampliada a **180px-210px** por carta con ventana de 1050px. | vitest **203/203** ✅; typecheck ✅; build ✅ |
| 2026-08-21 | Feature/UX | **Sistema Moderno de Filtrado de Mesas en el Lobby (`TableFilterBar.tsx`, `TableFilterBar.css`, `LobbyScreen.tsx`)**: (1) **Buscador Omnibox en vivo**: Filtrado por texto (mesa, creador, jugadores, formato, notas) con atajo `Esc` y botón de limpieza. (2) **Filtro de Formatos Multidimensional**: Chips para formatos principales (*Todos, Commander, Modern, Pioneer, Standard, Pauper, Limitado*) con contadores dinámicos y dropdown inteligente para formatos adicionales (*Legacy, Vintage, Brawl, Historic, Canadian Highlander, Freeform*). (3) **Filtros rápidos de disponibilidad**: Píldoras para *Plazas libres* y *En juego*. (4) **Panel Colapsable de Filtros Avanzados**: Modos (1v1, Multijugador/FFA, Torneo), Niveles (Novato, Casual, Competitivo) y Modificadores (Ocultar privadas, Solo Rated, Espectadores, Plazas IA). (5) **Estados vacíos contextuales**: Diferenciación entre "Servidor sin mesas" y "Filtros sin coincidencias" con botón de restablecimiento. | vitest **211/211** ✅; typecheck ✅; build ✅ |
| 2026-08-21 | Feature/UX | **Sala de Espera de Espectador (`SpectatorStagingScreen.tsx`, `SpectatorStagingScreen.css`, `App.tsx`, `store.ts`)**: (1) **Flujo de Espectador en Mesas en Espera**: Al pulsar *Ver* en una mesa que aún no ha comenzado (`WAITING` o `READY_TO_START`), el cliente transiciona al estado `phase: 'spectating_pending'`, abriendo una sala de espera interactiva en lugar de quedarse congelado. (2) **Arena Visual & Roster en Vivo**: Visualización de duelo 1v1 (Jugador 1 vs Jugador 2) o Pod de Commander con avatares, nombres, anfitrión 👑 y estado de conexión en tiempo real vía actualizaciones de mesa. (3) **Banner Animado & Chat**: Mensajes contextuales de pulso (*Esperando a que se completen las plazas...* / *Esperando inicio...*) y Chat integrado. (4) **Transición Automática al Tablero**: Al recibir `START_GAME` / `GAME_INIT`, conmuta sin fricción al tablero de juego en vivo. | vitest **214/214** ✅; typecheck ✅; build ✅ |
| 2026-08-21 | Fix/UI | **Saneamiento de Mensajes de Sistema en Chat Global & Filtro de Conexiones (`ChatBox.tsx`, `ChatBox.css`, `SpectatorStagingScreen.tsx`)**: (1) **Eliminación de dos puntos huérfanos (`: `)**: Parseo automático de avisos globales de XMage (`has joined`, `has lost connection`, `has disconnected`, `has left`), renderizándolos como píldoras de notificación del sistema con iconos (🟢, 🔌, 🚪) y texto en español. (2) **Filtro de notificaciones de conexión**: Añadido botón toggle `[ 👁️ Avisos visibles / 🔇 Avisos ocultos ]` para silenciar avisos de entrada/salida y ver exclusivamente el chat de jugadores. (3) **Clarificación en Sala de Espera**: Etiquetado explícito como *💬 Chat Global del Servidor* con subtítulo descriptivo. | vitest **217/217** ✅; typecheck ✅; build ✅ |
| 2026-08-21 | Fix | **Aislamiento de Chats y Limpieza al Abandonar Partida (`actions.ts`, `eventHandler.ts`, `ChatBox.tsx`, `commands.ts`)**: (1) **Comando `leaveChat`**: Añadido comando `cmds.leaveChat(chatId)` en el cliente y llamado automáticamente al salir de una partida (`returnToLobby`), desuscribiendo la sesión en el servidor para evitar recibir mensajes residuales de la partida abandonada. (2) **Depuración de historial de chat**: `returnToLobby` filtra `chatMessages` para eliminar mensajes de la partida finalizada y conservar únicamente el histórico de la sala global (`roomChatId`). (3) **Guarda de eventos `CHATMESSAGE`**: `eventHandler.ts` descarta mensajes provenientes de chats de partidas ajenas o inactivas cuando el usuario se encuentra en el lobby o la sala de espera. (4) **Filtro estricto en `ChatBox.tsx`**: Renderiza únicamente los mensajes que coinciden con el `chatId` de la sala activa. | vitest **219/219** ✅; typecheck ✅; build ✅ |
| 2026-08-21 | Feature/UX | **Selector Rápido de Mazo al Unirse a Mesas (`JoinTableDialog.tsx`, `JoinTableDialog.css`, `LobbyScreen.tsx`, `decks.ts`)**: (1) **Flujo de unión contextual tipo MTG Arena**: Al pulsar *Unirse (humano)* se abre un diálogo modal con los datos de la mesa (formato, modo, anfitrión, contraseña) y un selector rápido con todos los mazos guardados y predefinidos. (2) **Recomendación por formato**: Destaca visualmente los mazos recomendados para el formato (ej. 100 cartas para Commander, 60 para Modern). (3) **Importador exprés integrado**: Permite pegar e importar listas de texto (MTG Arena / Moxfield / Archidekt) directamente en el modal sin abandonar el lobby ni perder la plaza en la mesa. (4) **Gestión de contraseñas y feedback**: Entrada de clave protegida y visualización directa de errores de validación del servidor (`SHOW_USERMESSAGE`). | vitest **222/222** ✅; typecheck ✅; build ✅ |
