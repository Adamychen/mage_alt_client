...
const NAV_ITEMS: (SidebarIcon | 'sep')[] = [
  { id: 'back', label: 'Volver', path: ICON_PATHS.back },
  { id: 'play', label: 'Iniciar', path: ICON_PATHS.play },
  ...
  'sep',
  { id: 'exit', label: 'Salir', path: ICON_PATHS.exit, danger: true },
]
  ...
  return (
    <nav className="sidebar">
      ...
      {NAV_ITEMS.map((item, i) => {
        if (item === 'sep') return <div key={`sep-${i}`} className="sidebar-sep" />
        return (
          <button
            key={item.id}
            className={`sidebar-icon-btn ${item.danger ? 'danger' : ''} ${expanded === item.id ? 'active' : ''}`}
            title={item.label}
