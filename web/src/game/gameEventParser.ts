...
/** Strip XML/HTML tags and color tags like <font color=...> from XMage text */
...
export function parseGameEvent(
  ...
  if (turnMatch) {
    ...
    const pName = (turnMatch[2] || turnMatch[3] || '').trim()
    return {
      ...base,
      type: 'turn',
  ...
  // 2. Cast spells: "Player casts CardName [target: TargetName]" or "Player casts CardName"
  const castMatch = text.match(/^([^:]+?)\s+(?:casts|plays\s+spell)\s+([^\[\]]+?)(?:\s*\[target:\s*([^\]]+)\])?$/i)
  if (castMatch) {
  ...
  // 3. Play Lands: "Player plays LandName"
  const landMatch = text.match(/^([^:]+?)\s+plays\s+([A-Za-z0-9 ',/-]+)$/i)
  ...
  const attackMatch = text.match(/^([^:]+?)\s+attacks\s+with\s+(.+)$/i)
  if (attackMatch) {
    const pName = attackMatch[1].trim()
    const creatures = attackMatch[2].trim()
    return {
      ...base,
      type: 'attack',
      playerName: pName,
      isMe: isMe(pName),
      cardName: creatures,
      description: `${pName} ataca con ${creatures}`,
    }
  }
  ...
  // 8. Abilities: "Ability triggers: CardName" or "Player activates ability of CardName"
  ...
  const abilityActMatch = text.match(/^([^:]+?)\s+activates\s+(?:an\s+ability\s+of|the\s+ability\s+of)\s+([^-]+)/i)
  if (abilityActMatch) {
